...
class TestInterviewGraphIntegration:
    ...
    def test_interview_graph_state_transitions(self, sample_analyst, mock_llm, mock_web_search):
    ...
    @pytest.mark.slow
    def test_interview_graph_multiple_turns(self, sample_analyst, mock_llm, mock_web_search):
        ...
        expert_responses = [
            m for m in result["messages"] if hasattr(m, "name") and m.name == "expert"
        ]
...
class TestResearchGraphIntegration:
    ...
    def test_research_graph_build(self, mock_llm, mock_interview_graph):
        ...
        graph = build_research_graph(
            llm=mock_llm, interview_graph=mock_interview_graph, enable_interrupts=False
        )

        assert graph is not None
        assert hasattr(graph, "invoke")

    def test_research_graph_analyst_creation(self, mock_llm, mock_interview_graph):
        """Test analyst creation phase."""
        graph = build_research_graph(
            llm=mock_llm, interview_graph=mock_interview_graph, enable_interrupts=False
        )

        initial_state = create_initial_research_state(topic="AI Safety", max_analysts=2)
    ...
    def test_initiate_interviews_not_approved(self):
        ...
        state = {"topic": "AI Safety", "analysts": [], "human_analyst_feedback": "need changes"}
    ...
    @pytest.mark.slow
    def test_complete_research_workflow(self, mock_llm, mock_interview_graph):
        """Test complete research workflow end-to-end."""
        graph = build_research_graph(
            llm=mock_llm, interview_graph=mock_interview_graph, enable_interrupts=False
        )

        initial_state = create_initial_research_state(topic="Test Topic", max_analysts=1)
        initial_state["human_analyst_feedback"] = "approve"
...
class TestErrorScenarios:
    ...
    def test_interview_graph_llm_failure(self, sample_analyst, mock_failing_llm):
    ...
    def test_research_graph_missing_analysts(self, mock_llm, mock_interview_graph):
        """Test research graph with no analysts."""
        graph = build_research_graph(
            llm=mock_llm, interview_graph=mock_interview_graph, enable_interrupts=False
        )

        initial_state = create_initial_research_state(topic="Test", max_analysts=1)
    ...
    def test_interview_graph_search_failure(self, sample_analyst, mock_llm):
        """Test interview with search tool failure."""
        # Create mock that fails
        failing_search = Mock()
        failing_search.search.side_effect = Exception("Search failed")

        graph = build_interview_graph(llm=mock_llm, web_search_tool=failing_search)
...
class TestStatePersistence:
    ...
    def test_research_graph_with_checkpoint(self, mock_llm, mock_interview_graph):
    ...
    def test_state_recovery_after_error(self, mock_llm, mock_interview_graph):
        ...
        graph = build_research_graph(
            llm=mock_llm, interview_graph=mock_interview_graph, checkpointer=checkpointer
        )
...
class TestPerformance:
    ...
    def test_interview_execution_time(self, sample_analyst, mock_llm, mock_web_search):
    ...
    def test_parallel_interview_execution(self, sample_analysts, mock_llm, mock_interview_graph):
        ...
        initial_state = create_initial_research_state(
            topic="Test", max_analysts=len(sample_analysts)
        )
...
class TestGraphConfiguration:
    ...
    def test_interview_graph_detailed_prompts(self, sample_analyst, mock_llm):
        """Test interview graph with detailed prompts."""
        graph = build_interview_graph(llm=mock_llm, detailed_prompts=True)
    ...
    def test_research_graph_no_interrupts(self, mock_llm, mock_interview_graph):
    ...
    def test_research_graph_with_interrupts(self, mock_llm, mock_interview_graph):
...
class TestDataFlow:
    ...
    def test_interview_context_accumulation(
        self, sample_analyst, mock_llm, mock_web_search, mock_wikipedia_search
    ):
        """Test that context accumulates during interview."""
        graph = build_interview_graph(
            llm=mock_llm, web_search_tool=mock_web_search, wiki_search_tool=mock_wikipedia_search
        )
    ...
    def test_research_sections_aggregation(self, mock_llm, mock_interview_graph):
        ...
        def mock_sections_per_analyst(state):
            ...
            analyst_name = (
                ...
                else f"Analyst{call_tracker['count']}"
            )
        ...
        initial_state = create_initial_research_state(topic="Test", max_analysts=2)
    ...
    def test_message_flow_in_interview(self, sample_analyst, mock_llm):
...
class TestEdgeCases:
    ...
    def test_empty_search_results(self, sample_analyst, mock_llm):
        """Test handling of empty search results."""
        empty_search = Mock()
        empty_search.search.return_value = []
        empty_search.format_results.return_value = ""

        graph = build_interview_graph(llm=mock_llm, web_search_tool=empty_search)
    ...
    def test_very_short_interview(self, sample_analyst, mock_llm):
    ...
    def test_single_analyst_research(self, mock_llm, mock_interview_graph):
...
class TestRegressions:
    ...
    def test_section_source_deduplication(self, mock_llm, mock_interview_graph):
    ...
    def test_empty_interview_transcript_handling(self, sample_analyst, mock_llm):
