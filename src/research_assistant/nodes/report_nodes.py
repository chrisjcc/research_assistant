...
def write_section(
    ...
    system_message_content = format_section_instructions(
        analyst_focus=analyst.description,
        context=context_str,
...
def write_report(
    ...
    logger.debug(
        f"Synthesizing report: topic='{topic}', "
...
def assess_report_quality(state: ResearchGraphState) -> Dict[str, Any]:
    ...
    scores = {
        "completeness": (
            10 if validation.get("has_introduction") and 
            validation.get("has_conclusion") and 
            validation.get("has_sources") else 5
        ),
