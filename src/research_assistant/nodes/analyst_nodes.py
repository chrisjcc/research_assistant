...
def create_analysts(
    ...
    try:
        ...
        if not isinstance(perspectives, Perspectives):
            raise AnalystCreationError(
                f"Expected Perspectives object, got {type(perspectives)}"
            )
        ...
        for i, analyst in enumerate(analysts, 1):
            logger.debug(
                f"Analyst {i}: {analyst.name} - {analyst.role} at {analyst.affiliation}"
            )
...
def format_analysts_for_review(analysts: list[Analyst]) -> str:
    ...
    lines = [
        f"ANALYST TEAM ({len(analysts)} members)",
        "=" * 50,
        ""
    ]
