    ...
    if origin is Union:
        ...
        for arg in args:
            ...
            try:
                ...
                continue
        raise TypeError(
            f"{param_name} must be one of {args}, got {type(value).__name__}"
        )
    ...
    def wrapper(*args, **kwargs):
        ...
        for param_name, param_value in bound.arguments.items():
            ...
            try:
                ...
                raise TypeError(
                    f"Invalid argument in {func.__name__}(): {str(e)}"
                )
    ...
    def validate(self, value: Any, rule_name: str) -> bool:
        ...
        if not self.rules[rule_name](value):
            error_msg = self.error_messages.get(
                rule_name,
                f"Validation failed for rule: {rule_name}"
            )
...
def validate_list_length(
    ...
    if min_length is not None and length < min_length:
        raise ValueError(
            f"{param_name} must have at least {min_length} items, got {length}"
        )
    
    if max_length is not None and length > max_length:
        raise ValueError(
            f"{param_name} must have at most {max_length} items, got {length}"
        )
...
def validate_dict_keys(
    ...
    if required_keys:
        ...
        if missing:
            raise ValueError(
                f"{param_name} missing required keys: {', '.join(missing)}"
            )
    ...
    if required_keys or optional_keys:
        ...
        if invalid:
            raise ValueError(
                f"{param_name} has invalid keys: {', '.join(invalid)}"
            )
