    ...
    def execute(self, *args, **kwargs):
        ...
        for i, strategy in enumerate(self.strategies):
            try:
                ...
                if i > 0:
                    self.logger.info(
                        f"Succeeded with fallback strategy {i + 1}"
                    )
        ...
        raise Exception(
            f"All fallback strategies failed. Errors: {[str(e) for e in errors]}"
        )
...
def get_circuit_breaker_status(service: str) -> dict:
    ...
    return {
        ...
        "last_failure": circuit._last_failure_time.isoformat() if circuit._last_failure_time else None
    }
